...
describe('', function () {
  ...
  describe('Sessions and cookies', function () {
    ...
    it('Redirects to login after signup', function (done) {
      var options = {
        'method': 'POST',
    ...
    it('redirects to login page when it logs out', function (done) {
      addUser(function (err, res, body) {
        if (err) { return done(err); }
        var cookies = cookieJar.getCookies('http://127.0.0.1:4568/');
        var cookieValue = cookies[0].value;

        request('http://127.0.0.1:4568/logout', function (error, response, resBody) {
          if (error) { return done(error); }
          expect(response.req.path).to.equal('/login');
          done();
        });
  ...
  describe('Link creation:', function () {
    ...
    describe('Creating new links:', function (done) {
      ...
      //SECOND ADDIONTAL TEST
      it('adds multiple urls into the database', function(done) {
        ...
        requestWithSession(options, function (error, res, body) {
          ...
          requestWithSession(options2, function(error, res, body) {
            if (error) { return done(error); }
            db.query('SELECT * FROM links ', function (err, links) {
              var foundUrl;
              if (err) { return done(err); }
